...
function Form(props) {
  const { selectedUserPolicy, operation, userPolicy, users, policies, userIds, policyIds } = props;
...
// console.log(obj);
  ...
  const change = (e) => {
    var index = Object.values(userPolicy).findIndex((i) => {
      return i.policyName == e;
    });
    amountRef.current.value = Object.values(userPolicy)[index].amount;
    limitRef.current.value = Object.values(userPolicy)[index].limit;
  };
